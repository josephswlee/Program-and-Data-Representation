...
mergeSort:
	...
	cmp rsi, rdx 		; if left >= right)        
	jge sortingDone 	; then end the sorting
	xor r10, r10 		; reset r10
	push rsi 			; store left				1
	push rdx 			; store right				2
	add rdx, rsi 		; right + left
	shr rdx, 1 			; (right + left)/2
	mov r10, rdx 		; r10 = mid 
	push r10 			; store r10 				3
	call mergeSort 	; mergeSort (A, left, right)

	pop r10	 			; r10 = mid  			1
	pop rdx 			; rdx = right			2
	inc r10				; mid ++
	mov rsi, r10 		; mov r10 into rsi
	push rsi  			; store mid					4
	push rdx 			; store right 				5
	call mergeSort 	; mergeSort (A, mid+1, right)
	
	pop rdx 			; rdx = right 			3
	pop r10 			; r10 = mid				4
	pop rsi 			; rsi = left 			5
	mov rcx, rdx 		; rcx = right
	add rdx, rsi 		; right + left
	shr rdx, 1 			; rdx = mid 
	call merge 		; call merge (A, left, mid, right)
