...
//copy constructor
List::List(const List& sourse) //whenever dealing with an object
{
  head = new ListNode();
  tail = new ListNode();
  head -> next = tail;
  ...
  ListItr iter(sourse.head -> next); // head is empty so calling the next of head is calling the beginning
...
ListItr List::find(int x)
{
    ...
    else
    {
        ...
        ListItr notFound = tail; // because we need to access ListNode use functions
...
void List::removeAtTail() // this is for pop
{
    if(count != 0)
    {
        ListItr n = last(); // special case that we locate the last object here then remove that one
        n.current->previous->next = n.current->next; // set previous node's next to current's next
        n.current->next->previous = n.current->previous; // set next node's previous to current's previous
        delete n.current; // delete current
        count--; // decrement the count
